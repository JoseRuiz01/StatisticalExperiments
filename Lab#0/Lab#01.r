...
cv=apply(cereals[,c(4:10,12:13)],2,cv),
kurtosis=apply(cereals[,c(4:10,12:13)],2,kurtosis),
skew=apply(cereals[,c(4:10,12:13)],2,skewness))
#using it
...
head(arrange(cereals,desc(sugars)))

# If more than one variable is included, they are used to break the ties, if any.
...
cereals=mutate(cereals, logC=log(calories) )

# summarise and group_by
summarise(group_by(cereals,mfr), n=n(), mean(sugars), sd(sugars))

# sample_n and sample_frac: select random rows from the dataset
set.seed(12345)
sample_n(cereals, 10)
sample_data=sample_frac(cereals, 0.1)
