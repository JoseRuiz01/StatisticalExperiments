...
# Creating a new categorical variable for shell
cereals = cereals %>%
  mutate(shelf_2=fct_recode(
    shelf,
    "Low"="1",
    "Medium"="2",
    "High"="3"
...
bs+geom_point(aes(colour=shelf))
...
bs+geom_point(aes(color=fat, size=vitC, shape=shelf))
...
#more histograms
ggplot(data = cereals, aes(x = calories, after_stat(density), fill=shelf)) +geom_density()
ggplot(data = cereals, aes(x = calories, after_stat(density), colour=shelf)) +geom_freqpoly(binwidth=15)
...
# categorical variable. Since we cannot use "type", and "mfr" has many different values, let's see how to create a categorical variable 
# from a continuous one. Intervals [0,220),[220,321):
...
#barplots
# vitamins by shelf
ggplot(cereals,aes(x=shelf,fill=vitC))+geom_bar()
# changing the position of bars
ggplot(cereals,aes(x=shelf,fill=vitC))+geom_bar(position=position_dodge())
# distribution of fiber by shelf; not a good idea if you treat fiber as a continuous variable
...
## with data set hbat.csv, use of the function stat_summary()
...
# stat summary, using the function round to round continuos values, 
ggplot(hbat, aes(x=round(X6),y=X19))+geom_point(color="red")+stat_summary(fun.data="mean_cl_normal", geom="ribbon", alpha=0.1)

#Using the function round to plot the distribution of a continuous variable conditional on a continuous one.
ggplot(hbat, aes(x=round(X6),y=X19, group=1))+stat_summary(fun.data="mean_cl_normal", geom="ribbon", alpha=0.1)+geom_boxplot(aes(group=round(X6)), alpha=0.7)+geom_point(color="red")+scale_x_continuous(breaks=c(5:10))+
...
df1=cereals %>% group_by(shelf) %>% dplyr::summarize(N=n(),avg=mean(sugars), s=sd(sugars),se=s/sqrt(N),cil=avg-qt(0.975,N-1)*se,ciu=avg+qt(0.975,N-1)*se) 
