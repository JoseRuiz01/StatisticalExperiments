...
library(ggcorrplot) # yet a different way to visualize correlations
...
load("multivariate_descriptive.RData")
# Nice scatterplot matrix
...
ggcorrplot(cor(medifis[,-1]), hc.order = TRUE, type = "lower",
           lab = TRUE)
# The most complete representation of pairwise correlations
corr<-cor(medifis[,-1])
pvalues<-cor_pmat(medifis[,-1]) # p-values for correlation tests
ggcorrplot(corr, hc.order = TRUE,
           type = "lower", lab=TRUE, p.mat=pvalues)

#Partial correlations, package ppcor. Detailed output
pcor(medifis[,-1])
matrix.partial=pcor(medifis[,-1])$estimate
pvalues <- pcor(medifis[,-1])$p.value

#Visualizing partial correlations
corrplot.mixed(matrix.partial,order="AOE" )
ggcorrplot(matrix.partial, hc.order = TRUE,
           type = "lower", lab=TRUE, p.mat=pvalues)
...
# use this function on the dataset hbat
r2multv(hbat[,6:22])
...
# Effective dependence coefficient, 7 variables
...
base_11 <- base_1 %>% dplyr::select(wins, losses, points, field_goals, points3, free_throws,off_rebounds,
                             def_rebounds,assists, steals,blocks,personal_fouls)
1-det(cor(base_11))^{1/11}
eigen(cov(base_11))
r2multv(base_11[,c(3:6)])
ggpairs(base_11)

###### Testing normality assumptions. Package nortest=normality tests
...
# Just to choose one, we will definitely use Jarque-Bera's test when testing normality
...
# Why is  normality failing in variable hbat[,6]? Because of the kurtosis of the distribution
...
mvn(cbind(hbat[,6],log(hbat[,7])), mvnTest="mardia", multivariatePlot="qq", bc=T)
...
plot(outlier_618)
getFlag(outlier_618)
getCutoff(outlier_618)
table(getFlag(outlier_618))

###### Multivariate outliers, package mvn
library(mvoutlier)
...
interactive <- chisq.plot(hbat[,6:18])
