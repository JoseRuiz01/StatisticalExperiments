...
components <- data.frame(cereal_pca_r$ind$coord)
components$shelf=cereal.pc$shelf
ggpairs(components, aes(color = shelf, alpha = 0.5))
...
# labels for those points with cosine squared greater than 0.7, for example.
...
# showing unselected points
plot(cereal_pca_r, cex=0.7, shadow=TRUE, habillage=9, invisible="quali", select="cos2 0.7", unselect=0.5)
...
# Plotting the 3 variables that contribute the most to the representation 
...
ggplot(my.grid, aes(x=y, y=values))+geom_bar(stat="identity", aes(fill=x))+
  scale_fill_brewer(palette="Blues")
ggplot(my.grid, aes(x=x, y=values))+geom_bar(stat="identity", aes(fill=y))+
  scale_fill_brewer(palette="Blues")
...
# sum of the cos2 of the first variable along all dimensions
sum(cereal_pca_r$var$cos2[1,1:8])
# sum of the cos2 of the first individual along all dimensions
sum(cereal_pca_r$ind$cos2[1,])
###
# sum of the squared of the correlation between all variables and the first dimension
...
###
# sum of the contribution of all individuals to the first component
sum(cereal_pca_r$ind$contrib[,1])
# sum of the contribution of the first variable to all components
sum(cereal_pca_r$var$contrib[1,])
# sum of the contribution of all the variables to the first component
sum(cereal_pca_r$var$contrib[,1])
...
# For the first (second) component, are there any differences in the groups defined by the categorical variable?
...
# Toy example. Gene expression in 9 tumors. 9 genes. 
...
# Real example. Microarray data. 
# Expression levels of 6830 genes in 64 different tissues taken from cancerous tumors.
library(ISLR)
nci.data=NCI60$data
rownames(nci.data)=NCI60$labs
table(NCI60$labs)
rownames(nci.data)=abbreviate(rownames(nci.data))
# tissues are the variables, genes the individuals  
object_1=PCA(nci.data)
# tissues are the individuals, genes the variables
object_2=PCA(t(nci.data))

# Soccer data
soccer.pca <-PCA(la_liga, scale.unit = TRUE)
fviz_pca_ind(soccer.pca, pointsize = "cos2", 
             pointshape = 21, fill = "#E7B800",
             repel = TRUE # Avoid text overlapping (slow if many points)
)
fviz_pca_var(soccer.pca, col.var="contrib")+
  scale_color_gradient2(low="lightblue", mid="blue", 
                        high="darkblue", midpoint=55)+theme_bw()
...
mosaic(writers_table, shade=TRUE)
ca_writers=CA(writers_table,row.sup = c(16,17))
summary(ca_writers)
plot(ca_writers)
plot(ca_writers,selectRow="cos2 4", selectCol="cos2 3")

# Rows plot shows relationships between row points (texts)
fviz_ca_row(ca_writers, repel = TRUE)
fviz_ca_row(ca_writers, col.row = "cos2",
            gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"), 
            repel = TRUE)
# Column plot
fviz_ca_col(ca_writers)
fviz_ca_col(ca_writers, col.col = "cos2", 
            gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
            repel = TRUE)
# Biplot
# symmetric plot, global pattern within the data (default plot)
fviz_ca_biplot(ca_writers, repel = TRUE)

# Asymmetric plot to interpret the distance between rows and columns
fviz_ca_biplot(ca_writers, 
...
fviz_dend(hcpc_rows, show_labels = TRUE)
