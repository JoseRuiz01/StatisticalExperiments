...
mrses[1:5,]
# Number 4, married, 14 years old. Let's look her husband up in the data set:
which(str_detect(titanic$name, "Nasser"))
titanic[c(10,123),]
...
# a lot of values greater than, say, 12, which is the oldest boy in the Master group
...
# miss.alone distribution of age by pclass. Again 3rd class has the lowest
# survivability rate

ggplot(misses.alone, aes(x = age, fill = survived)) +
  facet_wrap(~pclass) +
  geom_histogram(binwidth = 5) +
  ggtitle("Age for 'Miss.Alone' by Pclass") +
  xlab("Age") +
  ylab("Total Count")
...
# Title could be predictive. Visualize survival rates by sibsp, pclass, and title
...
titanic$Family=car::recode(titanic$family.size, "1='Alone'; c(2,3,4)='Medium'; c(5,6,7,8,9,10,11)='Big' ")
titanic$Family=ordered(titanic$Family, levels=c("Alone", "Medium", "Big"))
table1=xtabs(~survived+Family, data=titanic)
table1
